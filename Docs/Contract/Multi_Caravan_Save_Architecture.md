...
```text
selectedCaravanId에 해당하는 Caravan에 Pending 존재 → Settlement
selectedCaravanId에 해당하는 Caravan이 Traveling → Traveling
selectedCaravanId에 해당하는 Caravan이 그 외 상태 → Preparation
```

다른 Caravan의 Pending 또는 Traveling 존재 여부는 selected Caravan의 화면 route를 바꾸지 않는다. 비선택 Caravan 완료는 pending 저장과 완료 entry별 Event/알림을 허용하지만 화면을 강제로 바꾸지 않는다. aggregate badge/경고 UX는 미결정이며 이 계약의 진행 대상 선택에 영향을 주지 않는다.

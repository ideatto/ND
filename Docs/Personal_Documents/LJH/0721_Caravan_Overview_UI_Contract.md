...
1. Framework/Caravan 기능 제작자가 Overview, Setting, Load Setting 및 TradePrepare 선택에 필요한 권위 있는 읽기 전용 조회 API를 제공한다.
2. UI & Data가 조회 API를 UI 소유 Provider 계약에 맞는 Adapter로 조립한다.
3. Framework가 `caravanId` 기반 조회·변경 Command를 제공한다.
4. 기존 S3 마차·동물 UI를 `CaravanSettingViewData` 입력 패널로 전환한다.

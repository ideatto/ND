...
- Provider는 슬롯 상태와 잠금 슬롯의 `unlockHintText`를 결정한다.
...
- 생성 요청 중인 Empty 슬롯은 UI의 일시 상태이며 Create/Setting/Cargo/Journey 입력을 비활성화한다.
...
- Empty와 Locked 슬롯은 Setting/Cargo/Journey 오브젝트를 숨겨 CreateButton이 블록 전체 폭을 사용한다. Locked는 이 Empty 기본 화면 위에 중앙 정렬된 LockOverlay를 덮고, 해금되어 Provider가 Empty를 반환하면 Overlay만 제거해 CreateButton을 활성화한다.

...
작성일: 2026-07-21
요청 영역: Framework/Core 및 기존 TradePrepare UI 연결 담당
요청 목적: 최대 4개의 실제 Caravan 중 이번 출발 대상을 TradePrepareUI에서 선택하고, 해당 `caravanId`가 출발·저장·진행·정산까지 유지되도록 연결한다.
...
- `CaravanOverviewViewData.focusedCaravanId`: Overview에서 설정·적재 편집 대상으로 집중한 ID
- `SaveData.selectedCaravanId`: 기존 단일 runtime 호환 접근자가 사용하는 Framework 선택 ID
- `TradePrepareDraft.departureCaravanId`: 이번 출발 요청이 대상으로 삼는 ID

세 값은 의미가 다르므로 자동 복사하거나 같은 상태로 취급하지 않는다.
...
## 3. 실제 Option Provider 구현 요청

Framework/Core 측에서 `ITradePrepareCaravanOptionProvider` 실제 구현을 제공한다.
...
## 4. `TradePrepareRuntimeContextProvider` 연결 요청

기존 RuntimeContext에 다음 접점이 필요하다.
...
RuntimeContext는 Overview의 `focusedCaravanId`를 출발 Draft에 넣지 않는다.

---

## 5. `CaravanSlotPanel` 연결 요청

현재 패널은 문자열 목록과 슬롯 index만 사용한다. 실제 Provider 모드에서는 다음 계약이 필요하다.
...
- Overview focus가 출발 대상으로 자동 사용되지 않는다.

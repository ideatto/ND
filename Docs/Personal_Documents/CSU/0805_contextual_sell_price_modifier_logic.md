...
**브랜치:** `Feature/economy/contextual-sell-price-modifiers`  
**베이스:** `dev2` (`27d04be`)  
**문서화 시점 상태:** 커밋 `c3c610f` 기준  
...
5. **Edit Mode 검증** — 선택기·preview/commit 일치·거리 경계·중복 제거 테스트
...
| 영역 | 파일 | 역할 |
|------|------|------|
| Context (신규) | `SellPriceCalculationContext.cs` | Season·거리·Lucky 입력 struct |
| Policy SO (신규) | `SellPriceModifierPolicy.cs` | 카테고리·Lucky·거리 규칙 정의 |
| Resolver (신규) | `ContextualSellPriceModifierResolver.cs` | item modifier + policy modifier 선택 |
...
| Sale session | `Assets/Scripts/UI/MarketInventoryIntegration.cs` | context 캡처·policy 주입·commit |
| UI entry | `Assets/Scripts/UI/Market/MarketTradePanelController.cs` | policy SO SerializeField·TryOpen 전달 |
...
Prepare → Traveling → Settle → SettlementPending
  ...
  → ConfirmSale / MarketTransactionCommand.Execute
       ...
       ResolveUnitPrices(item, context, policy)  ← 모든 라인 공유
...
- **패널 표시 단가**도 동일 context·policy를 사용하므로, 정책 SO가 연결된 경우 preview ≈ commit이다.
- policy가 `null`이면 item Season modifier + 기존 seasonal 선택만 적용된다 (contextual 효과 없음).
...
| 컴포넌트 | 책임 |
|----------|------|
| `SellPriceCalculationContext` | Season ID, route distance km, Lucky active 여부 (immutable) |
| `ContextualSellPriceModifierResolver` | item modifier + policy 규칙 → `PriceModifierInput` 목록 |
...
2. SeasonalSellPriceModifierSelector.SelectForSellPrice(itemModifiers, context.SeasonId)
3. if policy != null:
...
4. Deduplicate(modifiers)  // Type+SourceId+Target+Operation identity
...
MarketInventoryMutationSession.TryOpen(..., SellPriceModifierPolicy policy, ...)
  → session 필드에 policy 보관 (null 허용)
```

`MarketTradePanelController.OpenResolved`가 Inspector의 `sellPriceModifierPolicy`를 TryOpen에 전달한다.
...
1. **InGame Scene/Prefab 미배선** — `MarketTradePanelController.sellPriceModifierPolicy`가 아직 Prefab YAML에 연결되지 않음. 미등록 시 contextual 효과 없이 seasonal-only fallback.
...
- [ ] Edit Mode: `ContextualSellPriceTests` 전부 통과
...
- [ ] policy 미등록 → item Season modifier만 적용 (contextual 없음)

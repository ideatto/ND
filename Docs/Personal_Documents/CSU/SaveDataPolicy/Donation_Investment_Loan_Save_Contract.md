...
`RescueLoanSaveData` stores `loanId`, `originalPrincipal`, `remainingPrincipal`, `isActive`, `issuedUtcTicks`, and `isRestrictedPreparation`. It does not store a permanent prior-use flag or a loan phase in this contract. There is no interest, credit score, overdue penalty, settlement auto-repayment, or multiple simultaneous loan model.

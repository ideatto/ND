...
Catalog는 Sandbox 원본 데이터(`Assets/99.Sandbox/_LJH/`)를 이동하거나 수정하지 않고, Framework가 읽을 asset 참조 목록만 보관한다.
...
경로: `Assets/99.Sandbox/_LJH/02.SO/`
...
Player build에서는 `Resources` 폴더 하위 catalog asset이 필요하다.
...
2. Inspector에서 각 배열에 Sandbox ScriptableObject asset을 등록한다.
3. Play Mode 또는 `SharedDataTest` scene으로 검증한다.
...
| 로그 예시 | 원인 |
|-----------|------|
| `duplicate ID: dummytown` | 동일 Town asset 중복 등록 |
| `wagon reference is missing: wagondummymount` | Market이 참조하는 Wagon이 catalog에 없음 |
...
```text
[Framework] Shared game data loaded. Towns: 2, Markets: 1, ...
[SharedDataTest] SharedGameDataLoaded: Towns: 2, Markets: 1, ...
...
| `Assets/_Project/11.CoreServices/Scripts/Data/SharedGameDataService.cs` | 로드, 변환, 검증 |
...
- **Sandbox seed data, Catalog asset 내용:** Content & Tools, UI & Data 담당과 협의

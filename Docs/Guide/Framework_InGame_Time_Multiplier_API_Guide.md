...
| 식량 소모 (후속 PR) | `CaravanData.elapsedInGameSeconds` + `ToConsumptionPerInGameSecond` |
...
- Save v3 (`inGameTimeMultiplierAtStart`, `elapsedInGameSeconds`)
- 출발 시 배율 스냅샷, coordinator elapsed 저장
- Debug API, policy config 로드

### Core 후속 PR — 미완료

- `CaravanData.elapsedInGameSeconds` 필드 없음
- `CaravanCalculator.GetRemainingFood` → 아직 `progress01 × totalSeconds` (현실 초)
- `CaravanSaveDataMapper` → `elapsedInGameSeconds` runtime 매핑 없음
- coordinator가 `SetProgress` **후** SaveData에만 elapsed 기록

**배율을 올려도 식량 고갈 타이밍은 아직 현실 시간 기준**으로 동작한다.  
Core PR merge 전까지 식량 관련 테스트·밸런스는 이 점을 고려한다.

...
같은 version 5에 `world.marketInventories` / `world.marketPurchasePreparation`(상점 재고·구매 준비)와 `player.homeInventory` / `player.villageBuildings`(거점 창고·마을 건물)도 포함되며, 구 세이브의 null은 `JsonSaveService.NormalizeData`가 보정한다. version bump 없음.  

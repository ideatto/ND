...
public class MinimapClouds : MonoBehaviour
{
    ...
    /// 인자 = (simStep 결정론 축, 그 스텝의 벽시계 시각 초). 번개·낙뢰처럼 '날씨와 같은 시간축을
    /// 따라가는' 결정론 판정을 여기 붙이면, 되감기/투영에서도 같은 판정이 그대로 재생된다.</summary>
    public event System.Action<long, double> WeatherStepped;
    ...
    private void Update()
    {
        ...
        while (simStep < targetStep && guard < catchupCap)
        {
            ...
            simStep++;                         // ③ 순간 카운터 +1
            guard++;
            // ④ 이 스텝(simStep + 벽시계 시각) 통지 → 번개·낙뢰 결정론 판정이 날씨와 같은 시간축을 따라옴.
            if (WeatherStepped != null) WeatherStepped(simStep, weatherEpoch + simStep * fixedDt);
        }

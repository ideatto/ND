...
{
    public sealed class BuildingFunctionTutorialAdapter : TutorialChapterBehaviour
    {
        private const string FunctionOpened = "building.cottage_function_opened";
        private const string FunctionDismissed = "building.cottage_function_dismissed";
        private const string CollectAllSucceeded = "building.cottage_collect_all_succeeded";
        private const string WagonCollected = "building.cottage_wagon_collected";
        private const string DraftAnimalCollected = "building.cottage_draft_animal_collected";
        private const string ProductsCollected = "building.cottage_products_collected";
        ...
        private bool wagonCollected;
        private bool draftAnimalCollected;
        private bool collectAllSucceeded;
        ...
        private void Awake()
        {
            sequence = new TutorialSequenceRunner(new[] { FunctionOpened, ProductsCollected });
        ...
        private void HandleSignal(TutorialSignalStore.Signal signal)
        {
            ...
            if (signal.ActionId == FunctionDismissed)
            {
                sequence?.Reset();
                return;
            }

            if (signal.ActionId == FunctionOpened)
            {
                sequence?.Handle(FunctionOpened);
                TryCompleteCollectionStep();
                return;
            }

            if (signal.ActionId == CollectAllSucceeded)
                collectAllSucceeded = true;
            else if (signal.ActionId == WagonCollected)
                wagonCollected = true;
            else if (signal.ActionId == DraftAnimalCollected)
                draftAnimalCollected = true;
            else
                return;
        ...
        private void TryCompleteCollectionStep()
        {
            if (collectAllSucceeded || (wagonCollected && draftAnimalCollected))
                sequence?.Handle(ProductsCollected);
        }

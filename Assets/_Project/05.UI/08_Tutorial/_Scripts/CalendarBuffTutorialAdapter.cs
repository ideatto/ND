...
{
    public sealed class CalendarBuffTutorialAdapter : TutorialChapterBehaviour
    {
        private const string CalendarOpened = "calendar.opened";
        private const string SeasonBuffInspected = "buff.season_inspected";
        private const string DistanceBuffInspected = "buff.distance_inspected";
        private const string LuckyMoneyBuffInspected = "buff.lucky_money_inspected";

        private static readonly string[] OrderedActions =
        {
            CalendarOpened,
            SeasonBuffInspected,
            DistanceBuffInspected,
            LuckyMoneyBuffInspected
        };
        ...
        private void Awake()
        {
            sequence = new TutorialSequenceRunner(OrderedActions);
        ...
        private void HandleSignal(TutorialSignalStore.Signal signal)
        {
            ...
            int currentStep = sequence?.CurrentStepIndex ?? 0;
            if (currentStep >= OrderedActions.Length
                || signal.ActionId != OrderedActions[currentStep])
                return;
